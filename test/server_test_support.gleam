...
pub fn start_http_server() -> String {
  ...
  let _pid = process.spawn(fn() { start_server_process(reply_to) })
...
fn start_server_process(reply_to: process.Subject(Int)) -> Nil {
  let builder =
    mist.new(streamable_http.handler(sample_server()))
    |> mist.bind("127.0.0.1")
    |> mist.port(0)
